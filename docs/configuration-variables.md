...
This document explains the core variables used in the `terragrunt.hcl` files for deploying the My App infrastructure. 
...
*   **Current Value:** `"myapp"`
*   **Usage in Azure:** This variable is **not** used to name physical Azure resources directly. Instead, it is used strictly for **tagging**. Every resource created by the module will have the tag `app = myapp` applied to it.
...
The standard naming pattern is: `[resource_type_prefix] - myapp - [environment] - [region_shortcode]`
...
*   **`resource_group_name`**: Uses prefix `rg-` (e.g., `rg-myapp-prod-aue`)
*   **`container_app_environment_name`**: Uses prefix `cae-` (e.g., `cae-myapp-prod-aue`)
*   **`log_analytics_workspace_name`**: Uses prefix `law-` (e.g., `law-myapp-prod-aue`)
*   **`container_app_name`**: Uses prefix `ca-` (e.g., `ca-myapp-prod-aue`)
